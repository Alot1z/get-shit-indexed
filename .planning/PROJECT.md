...
A comprehensive enhancement of the GSD (Get Shit Done) system that fully integrates three MCP servers—Desktop Commander (DC), Code-Index MCP (CI), and CodeGraphContext (CG)—with optimal tool chain patterns. This system replaces native bash commands with MCP equivalents, enabling token-efficient workflow automation with built-in verification and debugging capabilities.

## Core Value

**Token-efficient, reliable GSD workflows that leverage all three MCP servers (DC + CI + CG) using proven tool chain patterns.**
...
- [ ] **MCP-INT-01**: All GSD workflows updated to use MCP tools instead of native bash
...
- [ ] **MCP-INT-06**: GSD commands at `C:\Users\mose\.claude\commands\gsd` updated for all 3 MCP servers
- [ ] **MCP-INT-07**: All research files (tool chain analysis) fully integrated into system
- [ ] **MCP-INT-08**: Cloned upstream repo at `C:\github-repos\my-claude-code-repos\get-shit-done-code-index` updated
...
**Existing Assets:**
- `C:\Users\mose\.claude\get-shit-done` — Local working directory with existing workflow updates (~95% MCP-integrated)
- `C:\Users\mose\.claude\commands\gsd` — GSD command definitions
- `C:\github-repos\my-claude-code-repos\get-shit-done-code-index` — Cloned upstream repo
- `C:\Users\mose\.claude\get-shit-done\implementing-using-code-index-mcp\` — Research directory with comprehensive analysis
- `C:\Users\mose\.claude\get-shit-done\references\` — Reference guides (questioning.md, ui-brand.md, templates)

**Research Completed:**
- **gsd-rewrite.txt** — Complete workflow rewrite with MCP tool patterns
...
- **Backward Compatibility**: Existing GSD commands must continue working
