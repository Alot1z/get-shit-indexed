...
- [x] **Task 1**: Create `commands/gsi/explorer.md` command definition
  ...
  - **COMPLETED**: Created `commands/gsi/explorer.md` with full MCP tool support

- [x] **Task 2**: Fork logic from `map-codebase.md`
  - Copy wave-based spawning pattern
  - Adapt for gap analysis instead of codebase mapping
  - Preserve MCP-only tool restrictions
  - **COMPLETED**: Created `workflows/explorer.md` with 4 parallel agent spawning pattern

- [x] **Task 3**: Define gap detection patterns
  - Pattern: Module exists but no invocations
  - Pattern: Native tool usage in GSI context
  - Pattern: Broken reference chains
  - **COMPLETED**: Defined in `commands/gsi/explorer.md` under `<gap_detection_patterns>`

- [x] **Task 4**: Create agent spawn configuration
  - Configure for parallel module scanning
  - Set up result aggregation
  - Define report generation format
  - **COMPLETED**: Created `agents/gsi-gap-explorer.md` with 4 focus areas

- [x] **Task 5**: Implement `--cxml` output format support
  ...
  - **COMPLETED**: Added `output_formats: [markdown, cxml]` and `<cxml_output_specification>` in workflow
