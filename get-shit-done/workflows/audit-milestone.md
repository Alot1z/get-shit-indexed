...
const INIT = await mcp__desktop-commander__start_process({
  command: "node ~/.claude/get-shit-done/bin/gsi-tools.js init milestone-op",
  timeout_ms: 10000
});
```
...
const CHECKER_MODEL = await mcp__desktop-commander__start_process({
  command: "node ~/.claude/get-shit-done/bin/gsi-tools.js resolve-model gsi-integration-checker --raw",
  timeout_ms: 10000
});
```
...
const phases = await mcp__desktop-commander__start_process({
  command: "node ~/.claude/get-shit-done/bin/gsi-tools.js phases list",
  timeout_ms: 10000
});
```
