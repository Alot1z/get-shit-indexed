...
await mcp__desktop-commander__start_process({
  command: "node ~/.claude/get-shit-done/bin/gsi-tools.js config-init",
  timeout_ms: 10000
});
```
...
```javascript
await mcp__desktop-commander__start_process({
  command: `node ~/.claude/get-shit-done/bin/gsi-tools.js config-set ${KEY} ${VALUE}`,
  timeout_ms: 10000
});
```
...
```javascript
await mcp__desktop-commander__start_process({
  command: `node ~/.claude/get-shit-done/bin/gsi-tools.js commit "chore: update GSD settings" --files .planning/config.json`,
  timeout_ms: 10000
});
```
