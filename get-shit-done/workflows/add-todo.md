...
const INIT = await mcp__desktop-commander__start_process({
  command: "node ~/.claude/get-shit-done/bin/gsi-tools.js init todos",
  timeout_ms: 10000
});
```
...
const slug = await mcp__desktop-commander__start_process({
  command: `node ~/.claude/get-shit-done/bin/gsi-tools.js generate-slug "$title" --raw`,
  timeout_ms: 10000
});
```
...
```bash
node ~/.claude/get-shit-done/bin/gsi-tools.js commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
