...
```bash
INIT=$(node ~/.claude/get-shit-done/bin/gsi-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
node ~/.claude/get-shit-done/bin/gsi-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/get-shit-done/bin/gsi-tools.js state update-progress

# Record execution metrics
node ~/.claude/get-shit-done/bin/gsi-tools.js state record-metric \
...
node ~/.claude/get-shit-done/bin/gsi-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/get-shit-done/bin/gsi-tools.js state add-blocker "Blocker description"
```
</step>
...
```bash
node ~/.claude/get-shit-done/bin/gsi-tools.js state record-session \
...
```bash
node ~/.claude/get-shit-done/bin/gsi-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/get-shit-done/bin/gsi-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
