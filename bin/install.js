...
function convertClaudeToOpencodeFrontmatter(content) {
  ...
  // Replace /GSI:command with /GSI-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/GSI:/g, '/GSI-');
 ...
 * OpenCode expects: command/GSI-help.md (invoked as /GSI-help)
 * Source structure: commands/GSI/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/GSI/)
 * @param {string} destDir - Destination directory (e.g., command/)
 * @param {string} prefix - Prefix for filenames (e.g., 'GSI')
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old GSI-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/GSI/debug/start.md -> command/GSI-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> GSI-help.md
...
/**
 * Clean up orphaned files from previous GSI versions
 */
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'GSI-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to GSI-statusline.js in v1.9.0
    'GSI-intel-index.js',  // Removed in v1.9.2
    'GSI-intel-session.js',  // Removed in v1.9.2
    'GSI-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  if (settings.statusLine && settings.statusLine.command &&
      ...
      !settings.statusLine.command.includes('GSI-statusline.js')) {
    // Replace old path with new path
    settings.statusLine.command = settings.statusLine.command.replace(
      /statusline\.js/,
      'GSI-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (statusline.js → GSI-statusline.js)`);
  }
...
/**
 * Uninstall GSI from the specified directory for a specific runtime
 * Removes only GSI-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling GSI from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 1. Remove GSI commands directory
  if (isOpencode) {
    // OpenCode: remove command/GSI-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if (file.startsWith('GSI-') && file.endsWith('.md')) {
  ...
  // 2. Remove get-shit-indexed directory
  const GSIDir = path.join(targetDir, 'get-shit-indexed');
  if (fs.existsSync(GSIDir)) {
    fs.rmSync(GSIDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed get-shit-indexed/`);
  }

  // 3. Remove GSI agents (GSI-*.md files only)
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} GSI agents`);
    }
  }
  ...
  if (fs.existsSync(hooksDir)) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} GSI hooks`);
    }
  }

  // 5. Clean up settings.json (remove GSI hooks and statusline)
  const settingsPath = path.join(targetDir, 'settings.json');
  if (fs.existsSync(settingsPath)) {
    let settings = readSettings(settingsPath);
    let settingsModified = false;

    // Remove GSI statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
        settings.statusLine.command.includes('GSI-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed GSI statusline from settings`);
    }

    // Remove GSI hooks from SessionStart
    if (settings.hooks && settings.hooks.SessionStart) {
      const before = settings.hooks.SessionStart.length;
      settings.hooks.SessionStart = settings.hooks.SessionStart.filter(entry => {
        if (entry.hooks && Array.isArray(entry.hooks)) {
          // Filter out GSI hooks
          const hasGSIHook = entry.hooks.some(h =>
            h.command && (h.command.includes('GSI-check-update') || h.command.includes('GSI-statusline'))
          );
          return !hasGSIHook;
        }
        return true;
      ...
      if (settings.hooks.SessionStart.length < before) {
        settingsModified = true;
        console.log(`  ${green}✓${reset} Removed GSI hooks from settings`);
      }
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove GSI permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('get-shit-indexed')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed GSI permissions from opencode.json`);
        }
      } catch (e) {
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No GSI files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} GSI has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading GSI reference docs
 * This prevents permission prompts when GSI accesses the get-shit-indexed directory
 */
function configureOpencodePermissions() {
  ...
  // Build the GSI path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const GSIPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/get-shit-indexed/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/get-shit-indexed/*`;
  ...
  if (config.permission.read[GSIPath] !== 'allow') {
    config.permission.read[GSIPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[GSIPath] !== 'allow') {
    config.permission.external_directory[GSIPath] = 'allow';
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for GSI docs`);
}
...
function writeManifest(configDir) {
  ...
  const GSIHashes = generateManifest(GSIDir);
  for (const [rel, hash] of Object.entries(GSIHashes)) {
    manifest.files['get-shit-indexed/' + rel] = hash;
  }
  if (fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/GSI/' + rel] = hash;
    }
  }
  if (fs.existsSync(agentsDir)) {
...
/**
 * Detect user-modified GSI files by comparing against install manifest.
 * Backs up modified files to GSI-local-patches/ for reapply after update.
 */
function saveLocalPatches(configDir) {
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified GSI file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Save any locally modified GSI files before they get wiped
  ...
  if (isOpencode) {
    ...
    // Copy commands/GSI/*.md as command/GSI-*.md (flatten structure)
    const GSISrc = path.join(src, 'commands', 'GSI');
    copyFlattenedCommands(GSISrc, commandDir, 'GSI', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/GSI-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('GSI-')).length;
  ...
  const skillSrc = path.join(src, 'get-shit-indexed');
  const skillDest = path.join(targetDir, 'get-shit-indexed');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime);
  if (verifyInstalled(skillDest, 'get-shit-indexed')) {
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    // Remove old GSI agents (GSI-*.md) before copying new ones
  ...
  const changelogDest = path.join(targetDir, 'get-shit-indexed', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'get-shit-indexed', 'VERSION');
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'GSI-statusline.js')
    : 'node ' + dirName + '/hooks/GSI-statusline.js';
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'GSI-check-update.js')
    : 'node ' + dirName + '/hooks/GSI-check-update.js';
  ...
  if (!isOpencode) {
    ...
    const hasGSIUpdateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('GSI-check-update'))
    );
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude') {
  ...
  const command = isOpencode ? '/GSI-help' : '/GSI:help';
  console.log(`
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  GSI includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with GSI statusline
`);
...
/**
 * Install GSI for all selected runtimes
 */
