...
function getCacheKey(toolName, context) {
  // Include configuration in cache key to handle overrides
  const { getConfiguration } = require('./mode-selector');
  const config = getConfiguration();
  // Only include non-null config values in the key
  const configParts = [];
  if (config.forceMode) configParts.push(`forceMode:${config.forceMode}`);
  if (config.forceServer) configParts.push(`forceServer:${config.forceServer}`);
  if (config.disableThinking) configParts.push(`disableThinking:${config.disableThinking}`);
  if (config.timeoutMultiplier !== 1.0) configParts.push(`timeoutMultiplier:${config.timeoutMultiplier}`);
  
  const configStr = configParts.join('|');
  const contextStr = JSON.stringify(context);
  return `${toolName}:${contextStr}:${configStr}`;
}
...
function clearCache() {
  cache.clear();
  // Also reset hit/miss counters for consistency
  metrics.hits = 0;
  metrics.misses = 0;
}
...
function selectThinkingMode(toolName, context = {}, useCache = true) {
  ...
  // Use original context for cache key (before enhancement)
  const cacheKey = useCache ? getCacheKey(toolName, context) : null;

  // Check cache first (before enhancement)
  ...
  try {
    ...
    // Cache result (using original context key)
    if (useCache && cacheKey) {
