 ...
 * 
 * Uses pattern-learning/storage for unified pattern storage.
 */

const { Pattern, PatternTypes } = require('./schema');
const { storePattern, getPatterns, getPatternsDir } = require('../pattern-learning/storage');
...
class PatternExtractor {
  constructor() {
    // Use unified pattern-learning storage
    this.patterns = [];
    this.loadPatterns();
  }
  
  /**
   * Load patterns from unified storage
   */
  async loadPatterns() {
    try {
      // Load from pattern-learning storage (all types)
      this.patterns = await getPatterns('all');
    } catch (error) {
  ...
  /**
   * Save pattern to unified storage
   */
  async savePatterns() {
    try {
      // Patterns are saved individually via storePattern in updateOrAddPattern
      return true;
    } catch (error) {
