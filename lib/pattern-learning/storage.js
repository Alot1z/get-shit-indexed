...
const { getPatternsPath, resolveDataPath } = require('../context/path-resolver');

/**
 * Get the patterns directory path (context-aware)
 * @returns {string} Absolute path to patterns directory
 */
function getPatternsDir() {
  return getPatternsPath();
}
...
async function storePattern(type, pattern) {
  try {
    ...
    const filePath = path.join(patternsDir, `${type}.json`);
