...
const TOOL_SERVER_MAP = {
  ...
  'mcp__CodeGraphContext__execute_cypher_query': 'CodeGraphContext',
  'mcp__CodeGraphContext__analyze_code_relationships': 'CodeGraphContext',
  'mcp__CodeGraphContext__find_code': 'CodeGraphContext',
  'mcp__CodeGraphContext__visualize_graph_query': 'CodeGraphContext',
...
function initializeServerHealth() {
  const servers = ['desktop-commander', 'code-index-mcp', 'CodeGraphContext',
...
function selectOptimalTool(operationType, context = {}) {
  ...
  switch (operationType) {
    ...
    case 'graph':
      result.recommended = 'mcp__CodeGraphContext__analyze_code_relationships';
      result.server = 'CodeGraphContext';
      result.tokenSavings = 60;
      
      // Check if CG is healthy
      if (!checkServerHealth('CodeGraphContext')) {
        result.fallback = getFallbackServer('CodeGraphContext');
        result.reasoning.push(`CodeGraphContext unavailable, fallback: ${result.fallback}`);
      }
      break;
