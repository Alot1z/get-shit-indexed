...
const { selectMode, getModeConfig, getAvailableModes, requiresThinking, getRecommendedMode, getThresholds, MODES, THRESHOLDS } = require('./mode-selector');
...
// Learning module for pattern capture
const { 
  recordEnhancement, 
...
// Context optimization for token awareness
let contextOptimization = null;
try {
  contextOptimization = require('../context-optimization');
} catch (e) {
  // Context-optimization may not be available
}
...
function analyzePrompt(prompt, options = {}) {
  ...
  // Token awareness using context-optimization
  let tokenCount = null;
  if (contextOptimization && contextOptimization.countTokens) {
    tokenCount = contextOptimization.countTokens(prompt);
  }
  ...
  return {
    ...
    template,
    tokenCount,
    complexityScore: riskScore, // Alias for knowledge flow
...
module.exports = {
  ...
  // Learning Module (knowledge flow integration)
  recordEnhancement,
  queryEnhancementPatterns,
  adaptEnhancementThreshold,
  loadHistory,
