// Phase 2: Process Assessment (Sequential + Code-Index MCP)
// Assesses process complexity using sequential thinking interleaved with file analysis

/**
 * Phase 2: Process Assessment
 * Uses Sequential thinking for multi-step process reasoning, interleaved with
 * Code-Index MCP for file summary and import analysis.
 *
 * Flow:
 * 1. Start sequential thinking for process assessment
 * 2. Use CI to analyze file summaries and imports
...
async function runProcessPhase(planMetrics, structureResult) {
  ...
  // Phase 2.2: Use Code-Index MCP for file analysis
  let dependencyCount = 0;
  let crossFileImpact = 0;

  if (files.length > 0) {
    try {
      // Analyze first file's imports as representative
      ...
      if (summary && summary.imports) {
        dependencyCount = summary.imports.length;
        crossFileImpact = Math.min(10, summary.imports.length / 2);
      }
      ...
      // CI might not be available - use heuristics
      console.debug(`Code-Index MCP unavailable for process phase: ${e.message}`);
