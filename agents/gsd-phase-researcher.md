...
Spawned by `/GSI:plan-phase` (integrated) or `/GSI:research-phase` (standalone).
...
<downstream_consumer>
Your RESEARCH.md is consumed by `GSI-planner`:
...
```bash
node ~/.claude/get-shit-indexed/bin/GSI-tools.js commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
