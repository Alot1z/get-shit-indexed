...
- write_file: Roadmap file creation
...
- `/GSI:new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your ROADMAP.md is consumed by `/GSI:plan-phase` which uses it to:
...
Use template from `~/.claude/get-shit-indexed/templates/roadmap.md`.
...
Use template from `~/.claude/get-shit-indexed/templates/state.md`.
